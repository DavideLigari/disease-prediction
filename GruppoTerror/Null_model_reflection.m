...
NULL_NEW_NUMB = 500; % number of null networks
%% kernel density
...
for t = 1:length(MM)
    ...
    for ng = 1:NULL_NEW_NUMB 
        ...
        RCA_bin(RCA_bin>0)=1;
        %% creo modello nullo
        ITER = 3; % iteration maslov rewiring
        RCA_null = RCA_bin; % inizio con modello nullo uguale a reale
        nn=size(RCA_null,1); % size
        mm=size(RCA_null,2);
        [i,j]=find(RCA_null); % trovo valori non vuoti
        K=length(i);
        ITER=K*ITER; % numero di iteraz 
        maxAttempts= round((nn+mm)*K/(nn*mm)); % maximal number of rewiring attempts per 'iter'
        eff = 0; % actual number of successful rewirings
        for iter=1:ITER
            att=0;
            while (att<=maxAttempts) %while not rewired                                    
                while 1
                    ...
                    if all(a~=c) && all(b~=d)
                        break           %funds and constit must be different
                    end
                end

                %rewiring condition
                if ~(RCA_null(a,d) || RCA_null(c,b))
                    RCA_null(a,d)=RCA_null(a,b); RCA_null(a,b)=0;
                    RCA_null(c,b)=RCA_null(c,d); RCA_null(c,d)=0;
    ...
    %% kernel density
    [f,xi] = ksdensity(Z_u(:,1),-10:.1:10);
    FU_kernel_mov(t,:)=f';
    XU_kernel_mov(t,:)=xi';
    
    [f,xi] = ksdensity(Z_v(:,1),-10:.1:10);
    FV_kernel_mov(t,:)=f';
    XV_kernel_mov(t,:)=xi';
...
for t = 1:length(MM)
    ZscoreU = [ZscoreU;Z_SCORE_u{t}(:,1)];
    ZscoreV = [ZscoreV;Z_SCORE_v{t}(:,1)];
...
SopraRappV = find(ZscoreV>1.65);
SottoRappV = find(ZscoreV<-1.65);
SopraRappU = find(ZscoreU>1.65);
SottoRappU = find(ZscoreU<-1.65);
...
TopV_sopra = TopV_sopra(1:10);
DoveTopV_sopra = DoveTopV_sopra(1:10);
NomiTopV_sopra=NodiV_sopraUnici(TopV_sopra);

[DoveTopU_sopra, TopU_sopra] = sort(sum(NodiU_sopraUniciZscore,2),'descend');
TopU_sopra = TopU_sopra(1:10);
DoveTopU_sopra = DoveTopU_sopra(1:10);
...
axis tight
title('Under-estimated HI level-1')
grid on
ylabel('Abs. z-score')
xlabel('Terrorists')
...
axis tight
title('Under-estimated RI level-1')
grid on
ylabel('Abs. z-score')
xlabel('Targets')
legend(string(num2str([1970:1992,1994:2019]')),'Orientation','horizontal','NumColumns',10,'location','southoutside')
...
axis tight
title('Over-estimated HI level-1')
grid on
ylabel('Abs. z-score')
xlabel('Terrorists')
legend(string(num2str([1970:1992,1994:2019]')),'Orientation','horizontal','NumColumns',10,'location','southoutside')
...
axis tight
title('Over-estimated RI level-1')
grid on
ylabel('Abs. z-score')
xlabel('Targets')
legend(string(num2str([1970:1992,1994:2019]')),'Orientation','horizontal','NumColumns',10,'location','southoutside')
...
zlabel('PDF')
title('HI level-1 Z-score distribution')
xtickangle(30)
ytickangle(330)
ax = gca;
...
xticklabels(string(num2str([1970:3:1992,1994:3:2019]')))
zlabel('PDF')
title('RI level-1 Z-score distribution')
xtickangle(30)
ytickangle(330)
ax = gca;
ax.ZAxis.Exponent = .02;
