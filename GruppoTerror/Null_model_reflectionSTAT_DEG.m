...
NULL_NEW_NUMB = 10; % number of null networks
%% kernel density
...
for t = 1:length(MM)
    ...
    %% max and samp
    output = Soluzioni{t,2};
    %% sampling   
    for ng = 1:NULL_NEW_NUMB             
       
       link = RCA(RCA>0); 
       while length(row)~=length(link)
           Wext=samplingAll(output,'DBCM');
...
%% save
save('pezzoNull_2VictimDEG.mat','FV_kernel_mov','FU_kernel_mov','XU_kernel_mov',...
