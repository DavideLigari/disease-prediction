...
% INPUT: a cell var with the names of the vertices (T), an adjacency matrix, [nxn], a filename, [string], node coordinates (optional)
...
function []=adj2pajek(T,adj,filename,x,y,z)

n = size(adj,1); % number of nodes 
fid = fopen(filename,'wt');

fopen(fid,'*Vertices  %6i\n',n);
if nargin <=3  % no coordinates specified - select random x,y,z=0
  for i=1:n
    fprintf(fid,'     %3i %s                     %1.4f    %1.4f   %1.4f\n',i,strcat('"',char(T(i)),'"'),rand,rand,0);
  end
elseif nargin >3 & nargin < 6  % 2D only, z is unspecified, put everything in one plane.
  for i=1:n
